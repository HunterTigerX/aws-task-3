...
describe('getProductsList Lambda handler', () => {
  ...
  test("should handle errors", async () => {
    ...
    jest.mock("./mockData", () => ({
      products: null, // Имитируем ошибку, передавая null вместо массива
    }));

    const event = {
      requestContext: {
        requestId: "test-request-id", // Добавляем requestId для проверки
      },
    };
    ...
    expect(body.requestId).toBe("test-request-id"); // Проверяем, что requestId добавлен
  });
});
