  ...
  try {
    ...
    if (forbiddenChars.test(fileNameWithoutExt)) {
      return {
        isValid: false,
        error:
          'Имя файла содержит недопустимые символы: <>:"/\\|?* и управляющие символы ASCII',
      };
    }
    ...
    if (!serviceMap.product) {
      ...
      if (method === "GET") {
        const cacheKey = `products_${
          isEmpty(query) ? "all" : Object.keys(query)[0]
        }`;
        // Check cache first
        ...
        if (!isEmpty(query)) {
          ...
          if (csvKey === "name" && checkCsv) {
            ...
            return {
              ...
              data: {
                message:
                  "invalid format of the file or invalid name or invalid key in query",
              },
            };
          }
        } else {
